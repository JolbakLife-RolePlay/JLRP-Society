local base64MoneyIcon = 'data:image/png;base64,'

RegisterNetEvent('JLRP-Framework:setJob')
AddEventHandler('JLRP-Framework:setJob', function(job)
	Core.PlayerData.job = job
	RefreshBussHUD()
end)

RegisterNetEvent('JLRP-Framework:playerLoaded')
AddEventHandler('JLRP-Framework:playerLoaded', function(xPlayer)
	Core.PlayerData = xPlayer
	Core.PlayerLoaded = true
	RefreshBussHUD()
end)

function RefreshBussHUD()
	DisableSocietyMoneyHUDElement()

	if Core.PlayerData.job.is_boss == true and Core.PlayerData.job.onDuty == true then
		EnableSocietyMoneyHUDElement()

		Core.TriggerServerCallback('JLRP-Society:getSocietyMoney', function(money)
			UpdateSocietyMoneyHUDElement(money)
		end, Core.PlayerData.job.name)
	end
end

RegisterNetEvent('JLRP-Addons-Account:setMoney')
AddEventHandler('JLRP-Addons-Account:setMoney', function(society, money)
	if Core.PlayerData.job and Core.PlayerData.job.is_boss == true and 'society_'..Core.PlayerData.job.name == society then
		UpdateSocietyMoneyHUDElement(money)
	end
end)

function EnableSocietyMoneyHUDElement()
	local societyMoneyHUDElementTpl = '<div>{{money}}&nbsp;<img src="' .. base64MoneyIcon .. '" style="width:20px; height:20px; vertical-align:middle;"></div>'

	if Core.GetConfig().EnableHud then
		Core.UI.HUD.RegisterElement('society_money', 5, 0, societyMoneyHUDElementTpl, {
			money = 0
		})
	end

	TriggerEvent('JLRP-Society:toggleSocietyHud', true)
end

function DisableSocietyMoneyHUDElement()
	if Core.GetConfig().EnableHud then
		Core.UI.HUD.RemoveElement('society_money')
	end

	TriggerEvent('JLRP-Society:toggleSocietyHud', false)
end

function UpdateSocietyMoneyHUDElement(money)
	if Core.GetConfig().EnableHud then
		Core.UI.HUD.UpdateElement('society_money', {
			money = Core.Math.GroupDigits(money)
		})
	end

	TriggerEvent('JLRP-Society:setSocietyMoney', money)
end

function OpenBossMenu(society, close, options)
	options = options or {}
	local elements = {}

	Core.TriggerServerCallback('JLRP-Society:isBoss', function(isBoss)
		if isBoss then
			local defaultOptions = {
				withdraw = true,
				deposit = true,
				wash = true,
				employees = true,
				grades = true
			}

			for k,v in pairs(defaultOptions) do
				if options[k] == nil then
					options[k] = v
				end
			end

			if options.withdraw then
				table.insert(elements, {label = _U('withdraw_society_money'), value = 'withdraw_society_money'})
			end

			if options.deposit then
				table.insert(elements, {label = _U('deposit_society_money'), value = 'deposit_money'})
			end

			if options.wash then
				table.insert(elements, {label = _U('wash_money'), value = 'wash_money'})
			end

			if options.employees then
				table.insert(elements, {label = _U('employee_management'), value = 'manage_employees'})
			end

			if options.grades then
				table.insert(elements, {label = _U('salary_management'), value = 'manage_grades'})
			end

			Core.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
				title    = _U('boss_menu'),
				align    = Config.MenuAlignment,
				elements = elements
			}, function(data, menu)
				if data.current.value == 'withdraw_society_money' then
					Core.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
						title = _U('withdraw_amount')
					}, function(data2, menu2)
						local amount = tonumber(data2.current.value)

						if amount == nil then
							Core.ShowNotification(_U('invalid_amount'))
						else
							menu2.close()
							TriggerServerEvent('JLRP-Society:withdrawMoney', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
				elseif data.current.value == 'deposit_money' then
					Core.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
						title = _U('deposit_amount')
					}, function(data2, menu2)
						local amount = tonumber(data2.current.value)

						if amount == nil then
							Core.ShowNotification(_U('invalid_amount'))
						else
							menu2.close()
							TriggerServerEvent('JLRP-Society:depositMoney', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
				elseif data.current.value == 'wash_money' then
					Core.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
						title = _U('wash_money_amount')
					}, function(data2, menu2)
						local amount = tonumber(data2.current.value)

						if amount == nil then
							Core.ShowNotification(_U('invalid_amount'))
						else
							menu2.close()
							TriggerServerEvent('JLRP-Society:washMoney', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
				elseif data.current.value == 'manage_employees' then
					OpenManageEmployeesMenu(society)
				elseif data.current.value == 'manage_grades' then
					OpenManageGradesMenu(society)
				end
			end, function(data, menu)
				menu.close()
			end)
		end
	end, society)
end

function OpenManageEmployeesMenu(society)
	Core.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. society, {
		title    = _U('employee_management'),
		align    = Config.MenuAlignment,
		elements = {
			{label = _U('employee_list'), value = 'employee_list'},
			{label = _U('recruit'), value = 'recruit'}
	}}, function(data, menu)
		if data.current.value == 'employee_list' then
			OpenEmployeeList(society)
		elseif data.current.value == 'recruit' then
			OpenRecruitMenu(society)
		end
	end, function(data, menu)
		menu.close()
	end)
end

function OpenEmployeeList(society)
	Core.TriggerServerCallback('JLRP-Society:getEmployees', function(employees)

		local elements = {
			head = {_U('employee'), _U('grade'), _U('actions')},
			rows = {}
		}

		for i=1, #employees, 1 do
			local gradeLabel = (employees[i].job.grade_label == '' and employees[i].job.label or employees[i].job.grade_label)

			table.insert(elements.rows, {
				data = employees[i],
				cols = {
					employees[i].name,
					gradeLabel,
					'{{' .. _U('promote') .. '|promote}} {{' .. _U('fire') .. '|fire}}'
				}
			})
		end

		Core.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			local employee = data.data

			if data.value == 'promote' then
				menu.close()
				OpenPromoteMenu(society, employee)
			elseif data.value == 'fire' then
				Core.ShowNotification(_U('you_have_fired', employee.name))

				Core.TriggerServerCallback('JLRP-Society:setJob', function()
					OpenEmployeeList(society)
				end, employee.citizenid, 'unemployed', 0, 'fire')
			end
		end, function(data, menu)
			menu.close()
			OpenManageEmployeesMenu(society)
		end)
	end, society)
end

function OpenRecruitMenu(society)
	Core.TriggerServerCallback('JLRP-Society:getOnlinePlayers', function(players)
		local elements = {}

		for i=1, #players, 1 do
			if players[i].job.name ~= society then
				table.insert(elements, {
					label = players[i].name,
					value = players[i].source,
					name = players[i].name,
					citizenid = players[i].citizenid
				})
			end
		end

		Core.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			title    = _U('recruiting'),
			align    = Config.MenuAlignment,
			elements = elements
		}, function(data, menu)
			Core.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				title    = _U('do_you_want_to_recruit', data.current.name),
				align    = Config.MenuAlignment,
				elements = {
					{label = _U('no'), value = 'no'},
					{label = _U('yes'), value = 'yes'}
			}}, function(data2, menu2)
				menu2.close()

				if data2.current.value == 'yes' then
					Core.ShowNotification(_U('you_have_hired', data.current.name))

					Core.TriggerServerCallback('JLRP-Society:setJob', function()
						OpenRecruitMenu(society)
					end, data.current.citizenid, society, 0, 'hire')
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		end, function(data, menu)
			menu.close()
		end)
	end)
end

function OpenPromoteMenu(society, employee)
	Core.TriggerServerCallback('JLRP-Society:getJob', function(job)
		local elements = {}

		for i=1, #job.grades, 1 do
			local gradeLabel = (job.grades[i].label == '' and job.label or job.grades[i].label)

			table.insert(elements, {
				label = gradeLabel,
				value = job.grades[i].grade,
				selected = (employee.job.grade == job.grades[i].grade)
			})
		end

		Core.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			title    = _U('promote_employee', employee.name),
			align    = Config.MenuAlignment,
			elements = elements
		}, function(data, menu)
			menu.close()
			Core.ShowNotification(_U('you_have_promoted', employee.name, data.current.label))

			Core.TriggerServerCallback('JLRP-Society:setJob', function()
				OpenEmployeeList(society)
			end, employee.citizenid, society, data.current.value, 'promote')
		end, function(data, menu)
			menu.close()
			OpenEmployeeList(society)
		end)
	end, society)
end

function OpenManageGradesMenu(society)
	Core.TriggerServerCallback('JLRP-Society:getJob', function(job)
		local elements = {}

		for i=1, #job.grades, 1 do
			local gradeLabel = (job.grades[i].label == '' and job.label or job.grades[i].label)

			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(gradeLabel, _U('money_generic', Core.Math.GroupDigits(job.grades[i].salary))),
				value = job.grades[i].grade
			})
		end

		Core.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
			title    = _U('salary_management'),
			align    = Config.MenuAlignment,
			elements = elements
		}, function(data, menu)
			Core.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				title = _U('salary_amount')
			}, function(data2, menu2)

				local amount = tonumber(data2.current.value)

				if amount == nil then
					Core.ShowNotification(_U('invalid_amount'))
				elseif amount > Config.MaxSalary then
					Core.ShowNotification(_U('invalid_amount_max'))
				else
					menu2.close()

					Core.TriggerServerCallback('JLRP-Society:setJobSalary', function()
						OpenManageGradesMenu(society)
					end, society, data.current.value, amount)
				end
			end, function(data2, menu2)
				menu2.close()
			end)
		end, function(data, menu)
			menu.close()
		end)
	end, society)
end

AddEventHandler('JLRP-Society:openBossMenu', function(society, close, options)
	OpenBossMenu(society, close, options)
end)

if Core.IsPlayerLoaded() then RefreshBussHUD() end